...
// integranda() = External Function defining Integrand.
// vk = Value of Integral using 41 nodes
// vg = Value of Integral using 20 nodes
void
krab(double a, double b, double(*integranda)(double), double* vk, double* vg) {
    const double wgb[] =
    {
    ...
    double s = b - a;
    *vg = 0.0;
    *vk = 0.08300356e-1 * integranda(a+0.5*s);

    for(int j=0; j<20; j++) {
        double add = s * abb[j];
        add = integranda(a+add) + integranda(b-add);
        *vk += (wkb[j] * add);
